...
if ( isset($_POST['btn-activation']) ) {
       ...
       $actcode = htmlspecialchars($actcode);
        ...
        $search = mysqli_query($conn, "SELECT code, active FROM dbuser WHERE code='".$actcode."' AND active='0'") or die(mysqli_error());
        $match  = mysqli_num_rows($search);

        if($match > 0){
            // We have a match, activate the account
            mysqli_query($conn, "UPDATE dbuser SET active='1' WHERE code='".$actcode."' AND active='0'") or die(mysqli_error());
